...
questions for a software engineering candidate. You will be given a target role, a difficulty level, \
and a few example questions that are already known to be good. Write NEW questions that match the \
same style, depth, and quality as the examples — do not repeat or lightly reword them. Each question \
must be realistic, specific enough to prompt a real answer, and appropriate for the stated difficulty \
level. Respond with strict JSON matching this shape: \
{"questions": [{"category": "behavioral" | "coding" | "system-design", "prompt": "..."}]}"""


def build_generate_questions_user_prompt(
    role: str, difficulty: str, count: int, exemplars: list[QuestionExemplar]
) -> str:
    exemplar_lines = "\n".join(
